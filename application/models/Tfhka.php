...
class Tfhka extends CI_Model 
{
	...
	public function __construct()
	{
		...
		if(!empty($this->Appconfig->get('fiscal_invoice_enable')) && $this->Appconfig->get('fiscal_invoice_enable'))
		{
			$address = $this->Appconfig->get('remote_ip_address');
			$service_port = $this->Appconfig->get('remote_port_service');

			$this->socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
			$this->address = $address;
			$this->service_port = $service_port;
			if ( $this->socket == false) {
				$this->log = "socket_create() falló: razón: ".socket_strerror(socket_last_error())."\n";
			} else {
				$result = socket_connect($this->socket, $address, $service_port);
				if ($result == false) {
					$this->log = "socket_connect() falló.\nRazón: ($result) " . socket_strerror(socket_last_error($this->socket)) . "\n";
					return false;
				} else {
					return true;
				}
