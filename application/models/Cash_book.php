...
class Cash_book extends CI_Model
{
	...
	public function get_info($cash_book_id)
	{
		$this->db->select("
				...
				CONCAT(people.first_name,' ',people.last_name,' (',employees.username,')') AS username
			");
		...
		$this->db->join('employees AS employees', 'cash_books.employee_id = employees.person_id');
	 ...
	 * @return array containing all the fields of the table row
	 */
	public function get_info_by_user($employee_id)
	{
		$query = $this->db->get_where('cash_books', array('employee_id' => $employee_id), 1);
	...
	*/
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'code', $order='asc', $count_only = FALSE)
	{
		...
		$this->db->group_start();
			...
			$this->db->or_like('employees.username', $search);
