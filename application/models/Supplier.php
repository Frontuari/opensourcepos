...
class Supplier extends Person
{
	...
	*/
	...
	Get search suggestions to find suppliers
	*/
	public function get_search_suppliers_suggestions($search, $limit = 25)
	{
		$suggestions = array();
		...
		foreach($this->db->get()->result() as $row)
		{
			$suggestions[] = array('value' => $row->person_id, 'label' => (!empty($row->company_name) ? $row->company_name : $row->first_name . ' ' . $row->last_name));
		}
