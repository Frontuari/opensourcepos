	...
	public function search_subconcept($cash_concept_parent_id ,$search, $rows = 0, $limit_from = 0, $sort = 'code', $order='asc', $count_only = FALSE)
	{
		...
		if($count_only == TRUE)
		{
			...
			$this->db->select(' concepts.*,
				...
				(SELECT name FROM ospos_cash_concepts WHERE cash_concept_id = '.$cash_concept_parent_id.') AS parent_name,
				(SELECT code FROM ospos_cash_concepts WHERE cash_concept_id = '.$cash_concept_parent_id.') AS parent_code');
		}
