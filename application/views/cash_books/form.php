...
<?php echo form_open('cash_books/save/'.$cash_book_info->cash_book_id, array('id'=>'cash_book_edit_form', 'class'=>'form-horizontal')); ?>
	<fieldset id="cash_books">
		...
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('cash_books_employee_id'), 'employee_id', array('class'=>'required control-label col-xs-3')); ?>
			<div class='col-xs-8'>
				<?php echo form_dropdown('employee_id', $employee, $selected_employee, array('class'=>'form-control', 'id' => 'employee_id')); ?>
			</div>
		</div>
...
$(document).ready(function()
{
	...
	$('#cash_book_edit_form').validate($.extend({
		...
		messages:
		{
			...
			employee_id: "<?php echo $this->lang->line('cash_book_employee_id_required'); ?>"
		}
	}, form_support.error));
	...
	$('#stock_location_id').change(function(){
		$.ajax({
			type: 'GET',
			url: "<?php echo site_url($controller_name.'/get_employee_by_location/"+$(this).val()+"'); ?>",
			dataType: 'json',
			success: function(resp){
				if(resp.length>0){
	            	...
	            	var value = -1;
	            	$('#employee_id').empty().append(new Option(text, value));
		            for(var i=0;i<resp.length;i++)
		            {
		            	var text = resp[i].first_name+" "+resp[i].last_name+" ("+resp[i].employeename+")";
		            	var value = resp[i].person_id;
		            	$("#employee_id").append(new Option(text, value));
		            }
				}
				else{
	            	$('#code').val("");
