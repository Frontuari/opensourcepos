...
class Cash_books extends Secure_Controller
{
	...
	*/
	...
	{
		$data_row = $this->Employee->get_employee_by_location($location_id);
		foreach(get_object_vars($data_row) as $property => $value)
		{
			$data_row->$property = $this->xss_clean($value);
		}
	...
	public function view($cash_book_id = -1)
	{
		...
		$employee = array('-1' => $this->lang->line('common_none_selected_text'));
		$data_row = $this->Employee->get_employee_by_location($data['cash_book_info']->stock_location_id);
		foreach($data_row as $row)
		{
			$employee[$this->xss_clean($row->person_id)] = $this->xss_clean($row->first_name." ".$row->last_name." (".$row->employeename.")");
		}
		$data['employee'] = $employee;
		$data['selected_employee'] = $data['cash_book_info']->employee_id;
